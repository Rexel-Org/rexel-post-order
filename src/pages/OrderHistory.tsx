...
function OrderCard({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "group cursor-pointer rounded-[var(--border-radius-sm)] border border-[#E0E4EB] bg-white p-[var(--spacing-3)] transition-shadow duration-150 hover:shadow-[0_8px_24px_-4px_rgba(0,0,0,0.18)]",
        warning &&
          "relative before:absolute before:left-0 before:top-0 before:bottom-0 before:w-[4px] before:bg-[var(--color-error)] before:rounded-l-[var(--border-radius-sm)]"
      )}
    >
...
export default function OrderHistory() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-[var(--color-border-subtle)]">
        {tabs.map((tab) => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={cn(
            "px-4 py-2.5 text-[12px] font-semibold transition-colors border-b-2 -mb-px",
      ...
      {/* Filter bar — sticky */}
      <div className="sticky top-[var(--flow-sticky-site-header-height,140px)] z-30 -mt-2 space-y-2 bg-[var(--color-bg-page)] pb-[var(--spacing-4)] pt-0 mb-[var(--spacing-4)]">
      ...
      {viewMode === "table" && (
        <div className="space-y-6">
          ...
          {(() => {
            ...
            const attentionPaginated = attentionRows.slice(0, 3);
                ...
                <div className="space-y-3">
                  {title && (
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="font-heading text-[18px] font-semibold leading-[22px] text-[var(--color-text-primary)]">
                        {title}{" "}
                        <span className="text-[var(--color-text-secondary)] font-semibold">({rows.length})</span>
                      </h2>
                    </div>
                  )}
                      ...
                      <tbody className="divide-y divide-[var(--color-border-subtle)]">
                        {displayRows.map((order) => {
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-[12px] text-[var(--color-text-secondary)]">{order.po_number ?? "—"}</td>
        ...
        <>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-heading text-[18px] font-semibold leading-[22px] text-[var(--color-text-primary)]">
                  ...
                  <span className="text-[var(--color-text-secondary)] font-semibold">({needsAttentionOrders.length})</span>
                </h2>
              </div>
              <div className="grid gap-3 grid-cols-1">
                {needsAttentionOrders.slice(0, 3).map((o) => (
          ...
          {activeTab === "ongoing" && ongoingOrders.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <h2 className="font-heading text-[18px] font-semibold leading-[22px] text-[var(--color-text-primary)]">
                  {t("orders.ongoing")}{" "}
                  <span className="text-[var(--color-text-secondary)] font-semibold">({ongoingOrders.length})</span>
                </h2>
              </div>
              <div className="grid gap-3 grid-cols-1">
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-[12px] text-[var(--color-text-secondary)] mb-4 max-w-sm">
          ...
          {(activeTab === "ongoing" ? ongoingOrders : filtered).length > visibleCount && (
            <div className="flex justify-center pt-2">
              <button
                ...
                className="inline-flex h-10 items-center gap-1.5 rounded-[var(--border-radius-sm)] border border-[var(--color-border-subtle)] px-6 text-[12px] font-semibold text-[var(--color-text-primary)] hover:bg-[var(--color-bg-layer-01)] transition-colors"
              >
